...
/**
 * Setting token thru navigating to home page and calling route interceptor with token injection
 */
test("logged in user can buy a product ", async ({
  context,
  page,
  request,
}) => {
  ...
  await context.route(
    (url) => {
      return url
        .toString()
        .startsWith("https://shopdemo-alex-hot.koyeb.app");
    },
    async (route, request) => {
      const headers = {
        ...request.headers(),
